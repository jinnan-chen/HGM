...
class ResUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.grad_checkpointing:
            ...
            x = checkpoint(self.upconv3,x3)

            x = checkpoint(self.skipconnect,x2, x)
            x = checkpoint(self.iconv3,x)

            x = checkpoint(self.upconv2,x)
            x = checkpoint(self.skipconnect,x1, x)
            x = checkpoint(self.iconv2,x)
            x_out = checkpoint(self.out_conv,x)
        else:
            x = self.relu(self.bn1(self.conv1(x)))
            x1 = self.layer1(x)
            x2 = self.layer2(x1)
